...
import m5 from './scenarios/m5-school-fee-structures.mjs';
...
dbTest('M5 (BL-03) rehearsal: fee structures attributed by label, cloned, collisions queued, lines linked, idempotent', async () => {
  const res = await runScenario(m5);
  assert.equal(res.idempotent, true, JSON.stringify(res.idempotencyChanges));
  assert.equal(res.ok, true, JSON.stringify(res.reconciliation.checks.filter((c) => !c.ok)));
});
