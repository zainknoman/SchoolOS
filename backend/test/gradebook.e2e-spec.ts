...
describe('Gradebook (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    for (const s of stale) {
      ...
      await prisma.timetable
        .deleteMany({
          where: { section: { class: { campus: { schoolId: s.id } } } },
        })
        .catch(() => undefined);
      await prisma.school
    ...
    if (staleSubject) {
      ...
      await prisma.subject
        .delete({ where: { id: staleSubject.id } })
        .catch(() => undefined);
    }
    ...
    const otherCampus = await prisma.campus.create({
      data: { schoolId: school.id, name: 'GB Other' },
    });
    ...
    const student = await prisma.student.create({
      data: { grNumber: 'GB-1001', name: 'GB Student' },
    });
    ...
    await prisma.studentParent.create({
      data: { studentId: ids.student, parentProfileId: parentProfile.id },
    });

    const otherStudent = await prisma.student.create({
      data: { grNumber: 'GB-OTHER-1', name: 'Other Student' },
    });
  ...
  afterAll(async () => {
    ...
    await prisma.mark
      .deleteMany({
        where: { studentId: { in: [ids.student, ids.otherStudent] } },
      })
      .catch(() => undefined);
    await prisma.studentParent
      .deleteMany({
    ...
    await prisma.student
      ...
      .catch(() => undefined);
    await prisma.assessment
      .deleteMany({ where: { subjectId: ids.subject } })
      .catch(() => undefined);
    await prisma.subject
      .delete({ where: { id: ids.subject } })
      .catch(() => undefined);
    await prisma.school
  ...
  describe('Terms', () => {
    it("SCHOOL_ADMIN creates a term and it appears in the session's list", async () => {
      ...
      const list = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${adminToken}`)
        .expect(200);
      expect(list.body.map((t: { label: string }) => t.label)).toContain(
        'Term 1',
      );
    });
  ...
  describe('Assessments', () => {
    it("a TEACHER in the class's campus creates an assessment; a different-campus TEACHER cannot", async () => {
      ...
      const res = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${teacherToken}`)
        .send({
          assessmentCategoryId: ids.categoryQuizzes,
  ...
  describe('Bulk marks', () => {
    ...
    it("rejects a studentId not enrolled in the assessment's class", async () => {
      ...
      await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${teacherToken}`)
        .send({
          marks: [{ studentId: 'not-a-real-student-id', obtainedMarks: 10 }],
        })
        .expect(400);
    });
  });
  ...
  describe('Grades computation', () => {
    it('computes a weighted final percentage across two categories, with an ungraded category contributing 0', async () => {
      ...
      const assessmentRes = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${teacherToken}`)
        .send({
          assessmentCategoryId: ids.categoryFinal,
          subjectId: ids.subject,
          label: 'Final Exam',
          maxMarks: 100,
        })
      ...
      const subjectGrade = res.body.find(
        (g: { subjectId: string }) => g.subjectId === ids.subject,
      );
