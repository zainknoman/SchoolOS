...
describe('Fees (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.user
      .deleteMany({ where: { identifier: { startsWith: 'fee-' } } })
      .catch(() => undefined);
    const staleStudents = await prisma.student.findMany({
      where: { grNumber: { startsWith: 'FEE-' } },
    });
    ...
    await prisma.student
      .deleteMany({ where: { grNumber: { startsWith: 'FEE-' } } })
      .catch(() => undefined);
    await prisma.feeStructure
      .deleteMany({ where: { name: 'FEE E2E Tuition' } })
      .catch(() => undefined);
    const stale = await prisma.school.findMany({
      where: { name: 'FEE E2E School' },
    });
    ...
    const section = await prisma.section.create({
      data: { classId: klass.id, name: 'FEE-A' },
    });

    const passwordHash = await argon2.hash(password);
    await prisma.user.create({
      data: {
        identifier: 'fee-admin@schoolos.edu.pk',
        passwordHash,
        role: 'SCHOOL_ADMIN',
        schoolId: school.id,
      },
    ...
    const teacherUser = await prisma.user.create({
      data: {
        identifier: 'fee-teacher@schoolos.edu.pk',
        passwordHash,
        role: 'TEACHER',
      },
    });
    ...
    const parentAUser = await prisma.user.create({
      data: {
        identifier: 'fee-parent-a@schoolos.edu.pk',
        passwordHash,
        role: 'PARENT',
      },
    });
    const parentBUser = await prisma.user.create({
      data: {
        identifier: 'fee-parent-b@schoolos.edu.pk',
        passwordHash,
        role: 'PARENT',
      },
    });
    ...
    const childA = await prisma.student.create({
      data: { grNumber: 'FEE-A1', name: 'FEE Child A' },
    });
    const childB = await prisma.student.create({
      data: { grNumber: 'FEE-B1', name: 'FEE Child B' },
    });
    ...
    const structure = await prisma.feeStructure.create({
      data: { name: 'FEE E2E Tuition', amount: 500000 },
    });
  ...
  afterAll(async () => {
    await prisma.feePaymentAllocation
      ...
      .catch(() => undefined);
    await prisma.feeVoucher
      .deleteMany({ where: { studentId: ids.childA } })
      .catch(() => undefined);
    await prisma.feeStructure
      .delete({ where: { id: ids.structure } })
      .catch(() => undefined);
    await prisma.student
    ...
    await prisma.user
      .deleteMany({
        where: {
          identifier: {
            in: [
              'fee-admin@schoolos.edu.pk',
              'fee-teacher@schoolos.edu.pk',
              'fee-parent-a@schoolos.edu.pk',
              'fee-parent-b@schoolos.edu.pk',
            ],
          },
        },
      })
  ...
  it('a parent pays a voucher end-to-end via the stub gateway webhook, and can then download the receipt PDF', async () => {
    ...
    const reference = new URL(
      `http://x${initiated.body.redirectUrl}`,
  ...
  it('a repeated webhook call for an already-completed payment is a no-op', async () => {
    ...
    const payments = await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${parentAToken}`)
      .expect(200);
    const matching = payments.body.filter((p: { voucherIds: string[] }) =>
      p.voucherIds.includes(voucherId),
    );
  ...
  it('staff records a cash payment against a voucher, and it appears completed with a receipt', async () => {
    ...
    expect(reconciled.body).toEqual(
      expect.objectContaining({ status: 'completed', method: 'cash' }),
    );
