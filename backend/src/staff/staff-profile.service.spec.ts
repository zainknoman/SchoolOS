...
describe('StaffProfileService', () => {
  ...
  describe('updateProfile', () => {
    it('throws NotFoundException when the staff member does not exist', async () => {
      prisma.staff.findUnique.mockResolvedValue(null);
      await expect(
        service.updateProfile('missing', { mobile: '0300-1112233' }, 'admin-1'),
    ...
    it('writes an audit log entry naming only the changed field keys, never their values', async () => {
      ...
      expect(prisma.auditLog.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({
            metadata: JSON.stringify({ fields: ['cnic'] }),
          }),
        }),
      );
    });
  ...
  describe('emergency contacts', () => {
    it('creates a contact scoped to the staff member', async () => {
      prisma.staff.findUnique.mockResolvedValue({ id: 'st1' });
      prisma.staffEmergencyContact.create.mockResolvedValue({
        id: 'ec1',
        name: 'Bushra Ahmed',
      });
    ...
    it('throws NotFoundException updating a contact that belongs to a different staff member', async () => {
      prisma.staffEmergencyContact.findUnique.mockResolvedValue({
        id: 'ec1',
        staffId: 'st-other',
        addressId: null,
      });
    ...
    it('deletes a contact scoped to the staff member', async () => {
      ...
      expect(prisma.staffEmergencyContact.delete).toHaveBeenCalledWith({
        where: { id: 'ec1' },
      });
    });
  });
  ...
  describe('experience', () => {
    it('creates an experience entry scoped to the staff member', async () => {
      prisma.staff.findUnique.mockResolvedValue({ id: 'st1' });
      prisma.staffExperience.create.mockResolvedValue({
        id: 'exp1',
      ...
      await service.createExperience(
        'st1',
        { organization: 'City Grammar School', role: 'Janitorial Staff' },
        'admin-1',
      );
    ...
    it('throws NotFoundException deleting an entry that belongs to a different staff member', async () => {
      prisma.staffExperience.findUnique.mockResolvedValue({
        id: 'exp1',
        staffId: 'st-other',
      });

      await expect(
        service.deleteExperience('st1', 'exp1', 'admin-1'),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('documents', () => {
    it('rejects adding a document whose fileId was never uploaded', async () => {
      ...
      await expect(
        service.addDocument(
          'st1',
          { documentType: 'CNIC', fileId: 'missing' },
          'admin-1',
        ),
    ...
    it('marks a document verified and records who verified it', async () => {
      prisma.staffDocument.findUnique.mockResolvedValue({
        id: 'doc1',
        staffId: 'st1',
      });
      prisma.staffDocument.update.mockResolvedValue({
        id: 'doc1',
