...
function makeRequest(path: string, accessToken?: string): Request {
  ...
  const url = new URL(`http://localhost${path}${accessToken ? `?access_token=${accessToken}` : ''}`);
  return { path, query, url: url.toString(), originalUrl: url.toString() } as unknown as Request;
}
