...
describe('StudentService', () => {
  ...
  it('rejects with a BadRequestException (not an unhandled error downstream) when parentProfileId is omitted and newParent is explicitly null', async () => {
    tx.student.create.mockResolvedValue({
      id: 's-mixed',
      grNumber: 'GR-2004',
      name: 'Mixed Null',
    });
  ...
  it('creates a Student + Enrollment (campusId derived from the section) linked to an existing parent, in one transaction', async () => {
    ...
    tx.parentProfile.findUnique.mockResolvedValue({
      id: 'p1',
    ...
    expect(tx.student.create).toHaveBeenCalledWith({
      data: { grNumber: 'GR-2001', name: 'New Student' },
    });
  ...
  it('creates a Student linked to a brand-new parent, via the same createParentWithUser logic, inside the same transaction', async () => {
    ...
    tx.parentProfile.create.mockResolvedValue({
      id: 'p-new',
      name: 'New Parent',
      phone: null,
    });
    ...
    const allAuditPayloads = tx.auditLog.create.mock.calls.map((call) =>
      JSON.stringify(call[0]),
    );
    expect(
      allAuditPayloads.some((payload) => payload.includes('ChangeMe123!')),
    ).toBe(false);
