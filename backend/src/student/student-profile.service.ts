...
export class StudentProfileService {
  ...
  async updateProfile(
    ...
    const data: Prisma.StudentUpdateInput = {
      ...
      ...(dto.leavingReason !== undefined
        ...
        : {}),
      ...(dto.studentMobile !== undefined
        ? { studentMobile: dto.studentMobile }
        : {}),
      ...(dto.studentEmail !== undefined
    ...
    let record: Prisma.StudentGetPayload<{ include: typeof PROFILE_INCLUDE }>;
    try {
  ...
  async updateCurrentEnrollment(
    studentId: string,
    dto: UpdateCurrentEnrollmentDto,
    actingUserId: string,
  ) {
    await this.requireStudent(studentId);
  ...
  async upsertPreviousSchool(
    studentId: string,
    dto: UpdateStudentPreviousSchoolDto,
    actingUserId: string,
  ) {
    await this.requireStudent(studentId);
    ...
    const scalarData = {
      ...
      leavingCertificateDate: dto.leavingCertificateDate
        ? new Date(dto.leavingCertificateDate)
        : undefined,
      reasonForLeaving: dto.reasonForLeaving,
      academicRemarks: dto.academicRemarks,
    };

    const existing = await this.prisma.studentPreviousSchool.findUnique({
      where: { studentId },
    });

    let record: Prisma.StudentPreviousSchoolGetPayload<{
      include: { address: true };
    }>;
    if (existing) {
  ...
  async upsertMedicalInfo(
    studentId: string,
    dto: UpdateStudentMedicalInfoDto,
    actingUserId: string,
  ) {
    await this.requireStudent(studentId);
  ...
  async createEmergencyContact(
    studentId: string,
    dto: CreateStudentEmergencyContactDto,
    actingUserId: string,
  ) {
    await this.requireStudent(studentId);
  ...
  async updateEmergencyContact(
    ...
    const existing = await this.prisma.studentEmergencyContact.findUnique({
      where: { id: contactId },
    });
  ...
  async deleteEmergencyContact(
    ...
    await this.prisma.studentEmergencyContact.delete({
      where: { id: contactId },
    });
    await this.prisma.auditLog.create({
      data: {
