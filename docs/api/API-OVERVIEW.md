...
| Errors | Nest default shape `{ "statusCode": n, "message": string \| string[], "error": "…" }`; 400 validation/rule, 401 auth, 403 role/scope, 404, 409 conflict, 429 throttled. **No global exception filter**, so unhandled failures return `500 Internal server error` with the default body | no `APP_FILTER`; `prisma-*-guard.ts` translate FK/unique errors |
