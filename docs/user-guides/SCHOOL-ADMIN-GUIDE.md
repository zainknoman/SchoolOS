...
4c. **Publish results:** on **Assessment Categories**, pick the class and term — the panel shows whether the results can be published (weights total 100 % and a default scale exists) → **Publish results**. Parents see a term's grades only after this. While published, categories and assessments are locked (marks can still be corrected); **Unpublish results** to change them. Letters stay as they were at publication even if the scale is edited later.
...
6. **Report Cards:** after publishing a class's results (4c), press **Generate report cards** on the same panel: every student of the class gets a card for the term (subjects, marks, %, grade, overall grade, GPA). Run it again after correcting marks — only students whose card changed get a new version; earlier versions are kept, marked *superseded*, and never altered. **Operations → Report Cards** shows a student's generated cards (with PDF) and still lets you attach an uploaded report-card document per session for historical cards.
...
## 5c. Attendance risk
**Operations → Attendance Risk** lists the students flagged right now and holds your school's settings: the window (days), the threshold (% of tracked school days absent — late counts as present, leave days and holidays are not counted), the minimum number of tracked days, and whether guardians are also alerted (off by default). When a student is newly flagged, the class teacher and the school's administrators get a notification. Changes apply from the next nightly check; only a school-wide administrator can change them.
