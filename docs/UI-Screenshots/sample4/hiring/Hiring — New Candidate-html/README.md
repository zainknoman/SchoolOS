# Hiring — New Candidate — design reference
...
- `HiringCandidateIntakeView.dc.html` — the artboard (a Design Component: an `<x-dc>`
...
Serve the folder (e.g. `python3 -m http.server`) and open `HiringCandidateIntakeView.dc.html`;
