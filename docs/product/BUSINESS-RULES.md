...
| BR-ATT-03 | **As implemented:** marking attendance requires a **Teacher identity** for the `Attendance.markedById` FK (a **required FK to `Teacher`**). A TEACHER is attributed directly; a SCHOOL_ADMIN/SUPER_ADMIN has no Teacher profile, so the write is attributed to the section's class teacher and is **refused when the section has none**. The real actor is recorded only in `AuditLog`. **Decided to change (RD-8): class teacher optional; add `markedByUserId` (actual actor); set `markedById` only for a Teacher; never synthesise a Teacher — BL-60 (schema M1).** | `attendance.service.ts` (`markAttendance`, `markBulk`, `resolveClassTeacherIdFrom`), `schema.prisma` `Attendance` | timetable-attendance |
...
| BR-LV-02 | Approving leave requires a class teacher on the student's section, because approval writes LEAVE attendance rows attributed to that teacher (same `markedById` FK as BR-ATT-03). **Decided to change (Q12): BL-29 (LeaveRequest recommender/decider fields, M1b) on top of BL-60.** | `leave.service.ts:109` | leave |
