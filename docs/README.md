...
| Database | [`database/`](database/DATA-MODEL.md) (model, generated dictionary + ERD, tenancy, migrations, [existing-school migration strategy (BL-62, approved)](database/MIGRATION-STRATEGY.md), seeding, history) | CURRENT (`data-model-design.md` and `migration-plan.md` are deprecated stubs kept only because code comments cite them) |
