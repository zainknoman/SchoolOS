...
**Progress (updated 2026-09-26 after BL-29, branch `wave-0/foundations`)** — delivered since this table was written (✅ = done, commit in EXECUTION-PLAN §0):
...
- ✅ Wave 5: BL-26 yearly syllabus per class + subject (admins edit, teachers of the class read, copied by copy-structure, ended sessions read-only); BL-27 per-school grading scales with letters/remarks/grade points and result publication gated on weights of 100 % (closes KI-16; also KG-26); BL-06 report cards generated from the published results (versioned, immutable, PDF, parents see the current card); BL-28 attendance-risk settings per school with alerts to teacher, admins and (optionally) guardians; BL-29 leave recommendation → decision, separately attributed and audited (M1b; also closed KG-27). Next: Wave 6 (BL-08 first), then Wave 7.
