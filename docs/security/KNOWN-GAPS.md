...
|---|---|---|---|---|
| KG-1 | **High** | **School-wide circulars are delivered to every parent in the database** (no school filter) | `circulars/circulars.service.ts:55-57` | TENANT-1 — *root cause:* `Circular` has no school anchor (only optional `sectionId`); fix needs `schoolId` (M2, BL-20) |
...
| KG-6 | High | Holidays: caller-supplied `campusId` unchecked on create; `campusId = null` applies to every campus of every school (holidays block attendance) | `holidays/holidays.service.ts:40-70` | TENANT-2 — *root cause:* `Holiday` has only a nullable `campusId`; fix needs `schoolId` (M2, BL-20) |
