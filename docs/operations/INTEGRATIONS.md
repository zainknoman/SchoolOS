...
| **File storage** | `S3StorageAdapter` (`STORAGE_DRIVER=s3`, BL-10) or `LocalDiskStorageAdapter` (dev/test) | `UPLOADS_DIR` | itself | persistent disk | n/a | read/write errors surface as 500 | Unit-tested | PARTIALLY IMPLEMENTED (no S3/object adapter) |
