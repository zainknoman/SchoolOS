...
| After deploying migration `20260925150000_m3_school_sessions` (BL-01) | Legacy sessions have no school | Run `npm run backfill:m3`. If it answers "M3 refused", a school would have two active sessions: deactivate the extra one(s) listed as `SESSION_MULTIPLE_ACTIVE`, then re-run. Afterwards resolve `SESSION_UNREFERENCED` / `SESSION_LABEL_COLLISION` rows (review-queue section) |
| After deploying migration `20260925180000_m4_school_subjects` (BL-02) | Legacy subjects have no school | Run `npm run backfill:m4` once; then resolve `SUBJECT_UNREFERENCED` rows (assign a school or delete the unused subject) |
| After deploying migration `20260925210000_m5_school_fee_structures` (BL-03) | Legacy fee structures have no school and are all ACTIVE | Run `npm run backfill:m5` once; resolve `FEE_STRUCTURE_NAME_COLLISION` / `FEE_STRUCTURE_UNREFERENCED` rows; confirm `FEE_STRUCTURE_ATTRIBUTED_BY_LABEL` rows |
| After deploying migration `20260926090000_bl32_staff_grants` (BL-32) | Accounts staff no longer see Admissions, Complaints or Messages | Intended: finance-only by default. A school admin re-grants the modules each person needs on **Operations → Accounts Access** (audited `account.grants`); the user signs in again to see the menu |
| After deploying migration `20260926150000_m6_guardian_links` (BL-04/BL-23) | Existing guardian links are all `OTHER` with no primary slot | Run `npm run backfill:m6` once (rules G4/G5); resolve `GUARDIAN_MULTIPLE_PRIMARY` rows by choosing two primaries on the parent screen; `GUARDIAN_PRIMARY_SLOT_REVIEW` rows are informational |
| After deploying migration `20260927090000_m7_lifecycle_statuses` (BL-61) | Students who left through a transfer still show "Left (needs review)" | Run `npm run backfill:m7` once (rules L1/L2); for each `LIFECYCLE_LEFT_MANUAL_REVIEW` row set the real final status (Transferred, Withdrawn or Graduated) on the student profile, then mark the row RESOLVED. `LEFT` is dropped from the enum only when no row uses it |
| After deploying migration `20260927150000_m8_teaching_assignment_history` (BL-25) | Teaching history is empty for assignments that existed before the deploy | Run `npm run backfill:m8` once; those rows show "before <session start>" (start unknown). New changes are recorded automatically by the database |
| Privacy request: permanently erase a person's record | Archive + erasure (BL-07) | Confirm the request with the privacy administrator (SUPER_ADMIN until RD-6 names the role); archive the record in the console; a SUPER_ADMIN calls `POST /api/v1/admin/<students|staff|teachers>/:id/erase`; a 400 means retained records still reference it — decide per the retention policy, never delete them by hand without approval; the audit log keeps `*.erase` |
| Before deploying migration `20260926120000_m12_db_invariants` (BL-53) | — | Run `npm run migration:dry-run` against the target DB; resolve every `M12_*` row (close extra ACTIVE enrolments, keep one voucher per student/session/month after moving payments, correct invalid statuses) |
| Migration `20260926120000_m12_db_invariants` failed with "M12 refused: …" | Existing rows violate an invariant; nothing was changed | Resolve the `M12_*` rows from the dry-run, then `npx prisma migrate resolve --rolled-back 20260926120000_m12_db_invariants` and `npx prisma migrate deploy` again |
