...
**M8 (BL-25) rehearsal** (`npm run migration:harness -- m8-teaching-assignments`, 2026-09-27): sections and timetables unchanged; one open row per class teacher and per distinct (section, teacher, subject), all flagged `startDateUnknown` with start = session start; label snapshots correct; then, each in a rolled-back transaction: a class-teacher change closes the old row and opens a new one; a timetable deleted and re-inserted unchanged opens/closes nothing; a deleted teacher's rows stay readable by name and are closed; rewriting a row or a closed row's end date is refused (23514); second backfill run changed nothing. All earlier scenarios still pass with the M8 triggers present. Deploy order: migration, then `npm run backfill:m8`.
