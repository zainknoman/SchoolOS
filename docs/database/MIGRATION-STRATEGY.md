...
**M8 (BL-25) rehearsal** (`npm run migration:harness -- m8-teaching-assignments`, 2026-09-27): sections and timetables unchanged; one open row per class teacher and per distinct (section, teacher, subject), all flagged `startDateUnknown` with start = session start; label snapshots correct; then, each in a rolled-back transaction: a class-teacher change closes the old row and opens a new one; a timetable deleted and re-inserted unchanged opens/closes nothing; a deleted teacher's rows stay readable by name and are closed; rewriting a row or a closed row's end date is refused (23514); second backfill run changed nothing. All earlier scenarios still pass with the M8 triggers present. Deploy order: migration, then `npm run backfill:m8`. **Follow-up `20260927170000_m8_history_fixes`** (found by the BL-07 e2e, reproduced in the scenario before the fix): removing a class teacher now closes the row (NULL-safe comparison), and deleting a class/school with open rows works (history foreign keys deferred to commit); scenario checks added, both failed before the fix. **M9 (BL-07/BL-63)** is additive only (archive columns, `RetentionPolicy` rows unset) — no backfill.
